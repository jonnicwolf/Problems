/**
 * We search non-negative integer numbers, with at most 3 digits, such as the sum of the cubes of their digits is the number itself; 
...
The task is to find, or not, the "cubic" numbers in the string and then to make 
 the sum of these "cubic" numbers found in the string, if any, and to return a string such as:
...
- s = "QK29a45[&erui9026315"
  ...
  Return: "Unlucky"
Notes
In the string "001234" where 3 digits or more follow each other the first group to examine is "001" and the following is "234".
 If a packet of at most three digits has been taken, whether or not "cubic", it's over for that packet.
...
function isSumOfCubes (s){
    ...
    for (let i=0; i<s.length; i++){
        ...
        if (s.charCodeAt(i)>47 && s.charCodeAt(i)<58){            
            ...
            if (s.charCodeAt(i+1)<48 || s.charCodeAt(i+1)>57){
                sumCubed = numberStack.map(num=>num = num**3).reduce((a,b)=>a+b)                
            }
            else if (numberStack.length===3){
                sumCubed = numberStack.map(num=>num = num**3).reduce((a,b)=>a+b)
            }
            console.log(sumCubed, numberStack)
            if (sumCubed===numberStack.join('')*1){
                result.push(sumCubed)
                numberStack.length = 0
            }        
        };                                    
    };            
    
    return result.length ? `${result.join('')} sumOfCubicNumbers Lucky` : `Unlucky`
}

// console.log(isSumOfCubes("QK29a45[&erui9026315"))
console.log(isSumOfCubes("153"))